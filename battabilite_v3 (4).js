...
function buildProfileChart(data, ztoe, blRef) {
  ...
  var ztoeStr  = ztoe  ? ztoe.toFixed(1)  : null;
  var blRefVal = blRef || 61;
  chartBC = new Chart(cv, {
    ...
    data:{ labels, datasets:[
      {
        ...
        borderWidth:2.5, pointRadius:2.5, tension:0.25, fill:true,
        order:2,
      },
      /* Ligne verticale critere de refus */
      {
    ...
    options:{ responsive:true, maintainAspectRatio:false, indexAxis:'y',
      plugins:{
        ...
        /* Ligne horizontale toe level via annotation manuelle afterDraw */
        afterDraw: null,
      },
      scales:{
        y:{ reverse:false,
  ...
  /* Dessiner la ligne horizontale toe level apres le rendu Chart.js */
  if (ztoeStr) {
    var origAfterDraw = chartBC.options.animation;
    cv.addEventListener('chartjs:afterDraw', function() {}, {once:true});
    /* Plugin inline pour ligne horizontale */
    var toeLine = {
      id: 'toeLine',
      afterDraw: function(chart) {
        ...
        var yPos = yScale.getPixelForValue(ztoeStr);
        ...
        ctx2.textAlign = 'left';
        ctx2.fillText('Toe level : ' + ztoeStr + ' mZH', xScale.left + 4, yPos - 4);
        ctx2.restore();
      }
    };
    chartBC.options.plugins = chartBC.options.plugins || {};
    Chart.register(toeLine);
    chartBC.update();
  }
}
