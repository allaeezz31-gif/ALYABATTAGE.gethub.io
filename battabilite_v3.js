...
function formulaGates(Eeff, Qult) {
  const K = 2500;
  return Math.max(1, Math.min(Math.round(K * Math.sqrt(Eeff) / Math.max(Qult, 1) * 10), 300));
}
...
function calc() {
  ...
  document.getElementById('res-content').innerHTML =
    ...
    '<div class="disclaimer">⚠️ <strong>Résultat estimatif.</strong> ' +
    'Méthode Gates calibrée sur simulation GRLWEAP — précision ±20 à 30%. ' +
    'Pour validation définitive : simulation GRLWEAP et essais PDA obligatoires.</div>' +
    ...
    /* Blow count — methode Gates calibree GRLWEAP */
    '<div class="bc-range">' +
      '<div class="bc-label">Blow count estimé — Méthode Gates (calibrée GRLWEAP)</div>' +
      '<div class="bc-vals">' +
        '<span class="bc-min">'+bl_gates+'</span>' +
        '<span class="bc-unit"> coups / 10 cm</span>' +
      '</div>' +
      '<div class="bc-methods">' +
        '<span class="bc-m">Énergie effective : <b>'+Eeff.toFixed(0)+' kJ</b></span>' +
        '<span class="bc-m">Q_ult : <b>'+(Qult/1000).toFixed(2)+' MN</b></span>' +
        '<span class="bc-m">K calibré GRLWEAP : <b>2500</b></span>' +
      '</div>' +
    '</div>' +
