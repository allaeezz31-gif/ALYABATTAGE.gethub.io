...
function formulaGates(Eeff, Qult) {
  const K = 7910; /* calibre exact sur GRLWEAP Zone II : 61 coups/10cm a -41.5 mZH */
...
function buildProfile(D, t, Ltot, As, peff, At, Eeff, M_ram_kg, qt, zcut, ztoe) {
  ...
  for (var z = zcut; z >= ztoe; z = Math.round((z - step) * 10) / 10) {
    ...
    /* Formule directe : bl = Qult × 10 / (Kd × √Eeff)
       Plus le sol est résistant, plus il faut de coups → courbe croissante */
    var Kd = 232.5; /* calibré : bl=61 à Qult=22428 kN, Eeff=250 kJ */
    var bl2 = Qult2 < 50 ? 1 : Math.max(1, Math.min(Math.round(Qult2 * 10 / (Kd * Math.sqrt(Eeff))), 300));
