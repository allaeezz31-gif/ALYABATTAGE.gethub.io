...
/* ============================================================
   FORMULES DE BATTABILITÉ — 3 méthodes indépendantes
   ============================================================ */

/**
 * Formule de Gates (1957) — Standard API RP2A offshore
 * bl = K × sqrt(Eeff_kJ) / Qult_kN × 10
 * Recommandée pour : grands pieux offshore (D > 600mm)
 */
function formulaGates(Eeff, Qult) {
  const K = 7036; /* calibré sur GRLWEAP Nador West Med Zone II */
  return Math.max(1, Math.min(Math.round(K * Math.sqrt(Eeff) / Math.max(Qult, 1) * 10), 999));
}

/**
 * Formule de Hiley (1930) — Standard NF P94-262 / EC7
 * Recommandée pour : pieux terrestres, petits/moyens diamètres
 * s = Eeff × eta_m / Qult − (c1+c2+c3)/2
 */
function formulaHiley(Eeff, Qult, D, t, Ltot, M_ram_kg, qt) {
  const As = Math.PI/4*(D*D-(D-2*t)*(D-2*t));
  const M_pieu = As * Ltot * 7850;
  const e_rest = 0.25;
  const c1_m = 0.006;
  const c3_m = (qt/1000)/2;
  const c2_m = Qult * 1000 / (As * 210e9);
  const eta_m = Math.max((M_ram_kg + e_rest*e_rest*M_pieu)/(M_ram_kg+M_pieu), 0.30);
  const s = Eeff * eta_m / Math.max(Qult, 1) - (c1_m+c2_m+c3_m)/2;
  if (s <= 0.0002) return 999;
  return Math.max(1, Math.min(Math.round(0.1/s), 999));
}

/**
 * Formule Rationnelle (Chellis 1961) — Standard FHWA / pratique générale
 * Recommandée pour : tous diamètres, pieux de refoulement
 * set = Eeff × eta_m / (Qult × fm)
 * fm = 1 + M_pieu / (3 × M_ram) — facteur masse pieu
 */
function formulaRational(Eeff, Qult, D, t, Ltot, M_ram_kg) {
  const As = Math.PI/4*(D*D-(D-2*t)*(D-2*t));
  const M_pieu = As * Ltot * 7850;
  const e_rest = 0.50; /* valeur IHC fabricant */
  const eta_m = (M_ram_kg + e_rest*e_rest*M_pieu)/(M_ram_kg+M_pieu);
  const fm = 1 + M_pieu/(3*M_ram_kg);
  const set_m = Eeff * eta_m / (Math.max(Qult,1) * fm);
  if (set_m <= 0.0002) return 999;
  return Math.max(1, Math.min(Math.round(0.1/set_m), 999));
}
...
function calc() {
  ...
  /* ── 3 formules blow count ── */
  const bl_gates    = formulaGates(Eeff, Qult);
  const bl_hiley    = formulaHiley(Eeff, Qult, D, t, Ltot, M_ram_kg, qt);
  const bl_rational = formulaRational(Eeff, Qult, D, t, Ltot, M_ram_kg);

  /* Fourchette et valeur centrale */
  const bl_min = Math.min(bl_gates, bl_hiley, bl_rational);
  const bl_max = Math.max(bl_gates, bl_hiley, bl_rational);
  const bl_moy = Math.round((bl_gates+bl_hiley+bl_rational)/3);
  ...
  if (bl_moy > 150 || !okS) {
    ...
    vd = bl_moy + ' coups/10 cm — ' + (FS_calc ? 'FS = ' + FS_calc.toFixed(2) + (FS_calc>=2.0?' ✓':' ⚠ FS < 2.0') + '. ' : '') + 'Sol battable dans les conditions simulées.';
  }

  function cc(ok,warn) { return ok?'c-ok':warn?'c-warn':'c-bad'; }

  /* ── Profil blow count par couche ── */
  const profileData = buildProfile(D, t, Ltot, As, peff, At, Eeff, M_ram_kg, qt, zcut, ztoe);

  /* ── HTML résultats ── */
  document.getElementById('res-content').innerHTML =
    /* Avertissement obligatoire */
    '<div class="disclaimer">⚠️ <strong>Résultat indicatif uniquement.</strong> ' +
    'Les formules empiriques (Gates, Hiley, Rationnelle) donnent une estimation de ±30 à 50%. ' +
    'Pour tout projet réel, une simulation GRLWEAP et des essais PDA sont indispensables.</div>' +

    /* Verdict */
    '<div class="verdict '+vc+'"><div class="v-ico">'+vi+'</div>' +
    '<div><div class="v-ttl">'+vt+'</div><div class="v-desc">'+vd+'</div></div></div>' +

    /* Fourchette blow count */
    '<div class="bc-range">' +
      '<div class="bc-label">Fourchette blow count estimé (3 méthodes)</div>' +
      '<div class="bc-vals">' +
        '<span class="bc-min">'+bl_min+'</span>' +
        '<span class="bc-sep"> – </span>' +
        '<span class="bc-max">'+bl_max+'</span>' +
        '<span class="bc-unit"> coups / 10 cm</span>' +
      '</div>' +
      '<div class="bc-methods">' +
        '<span class="bc-m">Gates : <b>'+bl_gates+'</b></span>' +
        '<span class="bc-m">Hiley : <b>'+bl_hiley+'</b></span>' +
        '<span class="bc-m">Rationnelle : <b>'+bl_rational+'</b></span>' +
      '</div>' +
    '</div>' +
    ...
    '</div>' +
    '<div class="res-row">' +
      '<div class="card full"><div class="card-ttl">Profil blow count vs profondeur (méthode Gates)</div>' +
...
/* ============================================================
   ...
   ============================================================ */
function buildProfile(D, t, Ltot, As, peff, At, Eeff, M_ram_kg, qt, zcut, ztoe) {
  const data = [];
  const step = 0.5; /* pas de 0.5m */
  /* Descendre par pas de 0.5m depuis zcut jusqu'à ztoe */
  for (var z = zcut; z >= ztoe; z -= step) {
    var Qs_cum = 0;
    layers.forEach(function(l) {
      var top = Math.min(l.from, zcut);
      var bot = Math.max(l.to, z);
      if (top <= bot) return;
    ...
    var qp_kPa2 = ((toeL2&&toeL2.qp)?toeL2.qp:0)*1000;
    var Qp2 = qp_kPa2 * At;
    var Qult2 = Qs_cum + Qp2;
    var bl2 = formulaGates(Eeff, Math.max(Qult2, 100));
    data.push({ z: z, bl: Math.min(bl2, 300) });
  }
  return data;
}
...
function buildProfileChart(data) {
  ...
  chartBC = new Chart(cv, {
    ...
    options:{ responsive:true, maintainAspectRatio:false, indexAxis:'y',
      plugins:{ legend:{display:false},
        tooltip:{callbacks:{
          label:function(ctx){ return ' '+ctx.parsed.x+' coups/10cm'; },
          title:function(items){ return 'Cote : '+items[0].label+' mZH'; }
        }}
      },
      scales:{
        y:{ reverse:false, title:{display:true, text:'Profondeur (mZH)', color:'#6b7280', font:{size:11}},
          ticks:{color:'#6b7280',font:{size:10}}, grid:{color:'rgba(0,0,0,0.05)'} },
        x:{ title:{display:true, text:'Blow count (coups/10cm)', color:'#6b7280', font:{size:11}},
          ticks:{color:'#6b7280',font:{size:10}}, grid:{color:'rgba(0,0,0,0.05)'} }
      }
    }
  });
