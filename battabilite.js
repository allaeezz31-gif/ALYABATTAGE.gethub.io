...
function calc() {
  ...
  if (isNaN(zcut) || isNaN(ztoe)) {
    alert('Vérifiez les cotes de battage.'); return;
  }
  if (ztoe >= zcut) {
    alert('La cote pied cible doit être inférieure à la cote de début de battage.'); return;
  }
  if ((zcut - ztoe) < 1.0) {
    alert('La pénétration minimale est de 1,0 m.'); return;
  }
  if (t_mm >= D_mm / 2) {
    alert('Épaisseur de paroi supérieure au rayon. Vérifiez la géométrie.'); return;
  }
  if (layers.some(function(l){ return isNaN(l.from)||isNaN(l.to)||l.from<=l.to; })) {
    alert('Vérifiez les cotes des couches (cote sup. > cote inf.).'); return;
  }
  ...
  const Eeff = Math.min(Wr*eta, 250);

  /* ── ÉTAPE 1 : Calcul Qult au toe level saisi (pour blRef) ── */
  var Qs_toe = 0;
  layers.forEach(function(l) {
    var top = Math.min(l.from, zcut);
    var bot = Math.max(l.to, ztoe);
    if (top <= bot) return;
    Qs_toe += (l.fs||0) * peff * (top - bot);
  });
  var toeL_init = null;
  for (var ii=layers.length-1; ii>=0; ii--) {
    if (ztoe <= layers[ii].from && ztoe >= layers[ii].to) { toeL_init=layers[ii]; break; }
  }
  var Qp_toe = ((toeL_init&&toeL_init.qp)?toeL_init.qp:0)*1000*At;
  var Qult_toe = Qs_toe + Qp_toe;

  /* ── ÉTAPE 2 : Critère de refus — calculé automatiquement ── */
  const blRef = formulaGates(Eeff, Qult_toe);

  /* ── ÉTAPE 3 : Détection refus anticipé — s'arrêter au premier bl >= blRef ── */
  var ztoe_eff = ztoe;
  var refus_anticipe = false;
  var step_pre = 0.5;
  for (var zz = zcut; zz >= ztoe; zz = Math.round((zz - step_pre)*10)/10) {
    var Qs_z = 0;
    layers.forEach(function(l) {
      var top2 = Math.min(l.from, zcut);
      var bot2 = Math.max(l.to, zz);
      if (top2 <= bot2) return;
      Qs_z += (l.fs||0) * peff * (top2 - bot2);
    });
    var tL_z = null;
    for (var jj=layers.length-1; jj>=0; jj--) {
      if (zz <= layers[jj].from && zz >= layers[jj].to) { tL_z=layers[jj]; break; }
    }
    var qp_z = (tL_z && tL_z.qp > 0) ? tL_z.qp*1000 : 0;
    var Qult_z = Qs_z + qp_z*At;
    var bl_z = Qult_z < 50 ? 1 : Math.max(1, Math.min(Math.round(Qult_z*10/(232.5*Math.sqrt(Math.max(Eeff,1)))), 300));
    if (bl_z >= blRef) {
      ztoe_eff = zz;
      if (Math.abs(zz - ztoe) > 0.3) refus_anticipe = true;
      break;
    }
  }

  /* ── ÉTAPE 4 : Calcul résistances sur ztoe_eff ── */
  var Qs = 0;
  const LR = [];
  layers.forEach(function(l) {
    var top = Math.min(l.from, zcut);
    var bot = Math.max(l.to, ztoe_eff);
    if (top <= bot) {
  ...
  for (var i=layers.length-1; i>=0; i--) {
    var ll=layers[i];
    if (ztoe_eff <= ll.from && ztoe_eff >= ll.to) { toeL=ll; break; }
  }
  if (!toeL) {
  ...
  if (bl_moy > 150 || !okS) {
    ...
    vd = bl_moy + ' coups/10 cm — ' +
      (!okS ? 'Contrainte dépassée (' + sigMPa.toFixed(0) + ' MPa > ' + (0.9*fy).toFixed(0) + ' MPa).'
            : 'Marteau insuffisant — choisir un marteau plus puissant.');
  } else if (!okB) {
    vc='v-warn'; vi='&#x26A0;'; vt='BATTABILITÉ MARGINALE';
    vd = bl_moy + ' coups/10 cm — Conditions difficiles. Vérification GRLWEAP recommandée.' +
      (fsWarn ? ' ⚠️ FS = ' + FS_calc.toFixed(2) + ' < 2,0.' : '');
  } else if (fsWarn) {
    vc='v-warn'; vi='&#x26A0;'; vt='BATTABLE — PORTANCE INSUFFISANTE';
    vd = bl_moy + ' coups/10 cm — Battage réalisable. ⚠️ FS = ' + FS_calc.toFixed(2) +
      ' < 2,0 requis. Augmenter la pénétration ou revoir le dimensionnement.';
  } else {
    vc='v-ok'; vi='&#x2713;'; vt='BATTABLE';
    vd = bl_moy + ' coups/10 cm — ' +
      (FS_calc ? 'FS = ' + FS_calc.toFixed(2) + ' ✓. ' : '') +
      'Sol battable dans les conditions simulées.' +
      (refus_anticipe ? ' ⚠ Refus prévisible à ' + ztoe_eff.toFixed(1) + ' mZH (avant ' + ztoe.toFixed(1) + ' mZH saisi).' : '');
  }
  ...
  document.getElementById('res-content').innerHTML =
    ...
    '<div class="mbox-grid">' +
      ...
      (refus_anticipe ?
        '<div class="mbox" style="border:1.5px solid #f59e0b;background:#fffbeb">' +
          '<div class="mb-lbl" style="color:#b45309">Refus prévisible à</div>' +
          '<div class="mb-val c-warn">'+ztoe_eff.toFixed(1)+'<span class="mb-unit">mZH</span></div>' +
        '</div>' : '') +
      '<div class="mbox"><div class="mb-lbl">Blow count (refus)</div>' +
    ...
    '</div>' +
    '<div class="res-row">' +
      '<div class="card full"><div class="card-ttl">Profil blow count vs profondeur</div>' +
  ...
  buildChart(LR, Qp);
  buildProfileChart(profileData, ztoe_eff, blRef);
  showTab(2);

  console.log('[calc] Qult='+Qult.toFixed(0)+' FS='+(FS_calc?FS_calc.toFixed(2):'N/A')+' bl='+bl_moy+' blRef='+blRef+' ztoe_eff='+ztoe_eff+' sigma='+sigMPa.toFixed(1));
}
  ...
  chartBC = new Chart(cv, {
    ...
    data: {
      ...
      datasets: [
        ...
        {
          label: 'Refus prévisible : ' + blRefVal + ' coups / 10 cm (calculé auto)',
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: function(ctx) {
              if (ctx.datasetIndex === 1) return ' Refus prévisible à ' + blRefVal + ' coups/10cm (calculé automatiquement)';
