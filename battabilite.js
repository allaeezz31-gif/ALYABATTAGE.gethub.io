...
function calc() {
  ...
  /* blRef calculé automatiquement après Qult — voir ci-dessous */
  ...
  if (ztoe >= zcut) { alert('La cote pied cible doit être inférieure à la cote TN (pénétration nulle).'); return; }
  ...
  /* Validation cote pied < cote début battage */
  if (ztoe >= zcut) {
    alert('La cote pied cible (' + ztoe + ' mZH) doit être inférieure à la cote de début de battage (' + zcut + ' mZH).\nVérifiez les valeurs saisies.');
    return;
  }
  /* Validation pénétration minimale */
  if ((zcut - ztoe) < 1.0) {
    alert('La pénétration minimale est de 1,0 m. Vérifiez les cotes saisies.\nCote début : ' + zcut + ' mZH\nCote pied : ' + ztoe + ' mZH');
    return;
  }

  if (blRef > 250) {
    alert('Le critère de refus (' + blRef + ' coups/10 cm) est irréaliste (maximum 250).\nValeur recommandée pour Zone II : 61 coups/10 cm.');
    return;
  }
  /* Validation épaisseur paroi vs diamètre */
  if (t_mm >= D_mm / 2) {
    alert('L\'épaisseur de paroi (' + t_mm + ' mm) est supérieure au rayon (' + (D_mm/2) + ' mm). Vérifiez la géométrie du pieu.');
    return;
  }
  ...
  /* ── Trouver la profondeur effective = min(z_refus, ztoe) ──
     Parcourir le profil par pas de 0.5m depuis zcut
     S'arreter des que bl(z) >= blRef : c'est le refus reel */
  var ztoe_eff = ztoe;        /* profondeur effective — peut être < ztoe si refus anticipé */
  var refus_anticipe = false;
  var bl_au_refus = null;
  var step_pre = 0.5;
  for (var zz = zcut; zz >= ztoe; zz = Math.round((zz - step_pre)*10)/10) {
    ...
    layers.forEach(function(l) {
      ...
      if (top2 <= bot2) return;
      Qs_pre += (l.fs||0) * peff * (top2 - bot2);
    });
    var tL_pre = null;
    for (var ii = layers.length-1; ii >= 0; ii--) {
      if (zz <= layers[ii].from && zz >= layers[ii].to) { tL_pre = layers[ii]; break; }
    }
    var qp_pre = (tL_pre && tL_pre.qp > 0) ? tL_pre.qp * 1000 : 0;
    var Qult_pre = Qs_pre + qp_pre * At;
    var bl_pre = Qult_pre < 50 ? 1 : Math.max(1, Math.min(Math.round(Qult_pre*10/(232.5*Math.sqrt(Math.max(Eeff,1)))), 300));
    if (bl_pre >= blRef) {
      ztoe_eff = zz;
      bl_au_refus = bl_pre;
      if (Math.abs(zz - ztoe) > 0.3) refus_anticipe = true; /* refus avant toe level saisi */
      break;
    }
  }

  /* ── Calcul résistance totale sur ztoe_eff ── */
  var Qs = 0;
  const LR = [];
  layers.forEach(function(l) {
  ...
  /* Résistance en pointe sur ztoe_eff */
  var toeL = null;
  ...
  const Qp = qp_kPa*At;
  const Qult = Qs+Qp;
  /* Critère de refus calculé automatiquement depuis Qult et Eeff */
  const blRef = formulaGates(Eeff, Qult);
  ...
  if (bl_moy > 150 || !okS) {
    ...
    var vd_base = bl_moy + ' coups/10 cm — ' + (FS_calc ? 'FS = ' + FS_calc.toFixed(2) + ' ✓. ' : '') + 'Sol battable dans les conditions simulées.';
    var vd_refus = refus_anticipe ?
      ' ⚠ Refus prévisible à ' + ztoe_eff.toFixed(2) + ' mZH (avant toe level saisi ' + ztoe.toFixed(2) + ' mZH).' : '';
    vd = vd_base + vd_refus;
  }
  ...
  /* Toe level effectif pour le graphique = profondeur de refus réelle */
