...
/* ─── PRESET DATA ─── */
const PDATA = {
  // Zone I  — CH 600–900  — Top Tuf : −27 mZH
  nadorZ1: [
  ...
  // Zone II — CH 900–1120 — Top Tuf : −30 mZH
  nadorZ2: [
  ...
  // Zone III— CH 1120–1250— Top Tuf : −33 mZH
  nadorZ3: [
  ...
  // Zone IV — CH 1250–1440— Top Tuf : −37 mZH
  nadorZ4: [
...
function calc() {
  ...
  // ── BLOW COUNT (Hiley simplifiée — NF P 94-262 / Eurocode 7 Annexe D) ──
  // E_eff = Q_ult × (set + q_avg)  →  set = (E_eff × etaTrans) / Q_ult − q_avg
  const qtm      = qt / 1000;   // mm → m
  const qsm      = qs / 1000;   // mm → m
  const qavg     = Qult > 0 ? (Qs * qsm + Qp * qtm) / Qult : qtm;
  const etaTrans = 0.90;        // Efficacité de transmission marteau→pieu (hydraulique)
  ...
  // ── COMPRESSION STRESS (borne supérieure — formule simplifiée équation des ondes) ──
  // Note: Surestime la contrainte réelle (ignore amortissement sol et impédance).
  // Pour dimensionnement final : utiliser GRLWEAP ou code aux ondes complet.
  const Esteel  = 210e6;   // kPa
  const sigkPa  = Math.sqrt(2 * Eeff * Esteel / (As * Ltot));
  const sigMPa  = sigkPa / 1000;

  // ── DEBUG — vérification dans la console du navigateur (F12) ──
  console.log('[calc] BILAN BATTABILITÉ', {
    'D (m)': D.toFixed(3), 't (m)': t.toFixed(4), 'plug': plug,
    'zcut (mZH)': zcut, 'ztoe (mZH)': ztoe,
    'As (m²)': As.toFixed(4), 'At (m²)': At.toFixed(4), 'peff (m)': peff.toFixed(3),
    'Wr (kJ)': Wr, 'η': eta, 'Eeff (kJ)': Eeff.toFixed(1),
    'Qs (kN)': Qs.toFixed(0), 'Qp (kN)': Qp.toFixed(0), 'Qult (kN)': Qult.toFixed(0),
    'qavg (mm)': (qavg*1000).toFixed(2),
    'set s (mm/coup)': (s*1000).toFixed(2),
    'Coups/10cm': bl >= 999 ? '>999' : bl,
    'σ_max (MPa)': sigMPa.toFixed(1), '0.9·fy (MPa)': (0.9*fy).toFixed(0),
  });
